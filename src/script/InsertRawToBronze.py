#!/usr/bin/env python
# InsertRawToBronze.py - Simple CSV to Iceberg ETL
import os
import re
import logging
...
def read_all_csv_in_folder(spark: SparkSession, folder_path: str) -> Optional[DataFrame]:
    ...
    csv_path_pattern = os.path.join(folder_path, "*.csv")
    logger.info(f"Attempting to read all CSV files matching pattern: {csv_path_pattern}")
    
    try:
        # Use permissive mode with a multiline option to handle quoted fields that may span lines
        df = (
            spark.read
            .option("header", "true")
        ...
        if df.rdd.isEmpty():
            logger.warning(f"No data found in CSV files at {folder_path}")
            return None
        ...
        logger.error(f"Error reading CSV files from {folder_path}: {str(e)}")
        return None
